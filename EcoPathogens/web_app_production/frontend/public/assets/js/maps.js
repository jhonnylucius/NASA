...
class MapsManager {
    constructor() {
        ...
        // ✅ VALIDAÇÃO ROBUSTA - Não inicializa se elemento não existir
        if (!this.mapElement) {
            console.error('❌ ERRO CRÍTICO: Elemento #interactive-map não encontrado no DOM');
            console.error('   Verifique se o HTML contém: <div id="interactive-map"></div>');
            return; // Para aqui - não tenta inicializar
        }

        // Verifica se Leaflet está carregado
        if (typeof L === 'undefined') {
            console.error('❌ ERRO CRÍTICO: Leaflet (L) não está definido');
            console.error('   Verifique se o script do Leaflet foi carregado no HTML');
            this.showMapError('Biblioteca de mapas (Leaflet) não foi carregada');
            return;
        }
    ...
    init() {
        ...
        if (!this.mapElement) {
            ...
            this.showMapError('Elemento #interactive-map não encontrado no DOM');
            return;
        }
        ...
        if (!isVisible) {
            console.warn('⚠️ AVISO: Elemento #interactive-map existe mas está OCULTO');
            console.warn('   Dimensões:', rect.width, 'x', rect.height);
            console.warn('   O mapa pode não inicializar corretamente.');
            console.warn('   Aguardando elemento ficar visível...');

            // Tenta novamente quando ficar visível
            this.waitForVisibleElement();
            return;
        }
        ...
        // ✅ NÃO chama loadRealAmazonData aqui - será chamado após mapa estar pronto
    }
    ...
    waitForVisibleElement() {
        ...
        const checkVisibility = () => {
            ...
            if (isVisible) {
                console.log(`✅ Elemento visível após ${attempts} tentativas - inicializando mapa...`);
                this.setupMapFilters();
                this.initializeMap();
            } else if (attempts >= maxAttempts) {
                console.error('❌ TIMEOUT: Elemento #interactive-map não ficou visível após 10 segundos');
                this.showMapError('Elemento do mapa não está visível. Verifique a introdução cinemática.');
            } else {
    ...
    async loadRealAmazonShape() {
        ...
        if (this.layers.amazonShape) {
            this.map.removeLayer(this.layers.amazonShape);
        }
    ...
    loadFallbackAmazonData() {
        // ❌ REMOVIDO FALLBACK - Conforme solicitado pelo usuário
        console.error('❌ ERRO: Não foi possível carregar dados da Amazônia');
        console.error('   APIs externas podem estar indisponíveis ou com erro');
        console.error('   SEM FALLBACK - mostrando erro real');

        this.showMapError(
            '❌ Falha ao carregar dados da Amazônia<br><br>' +
            'Possíveis causas:<br>' +
            '• Overpass API indisponível<br>' +
            '• Problema de conexão<br>' +
            '• Limite de rate da API atingido<br><br>' +
    ...
    initializeMap() {
        try {
            ...
            // ✅ VERIFICAÇÃO CRÍTICA: Se mapa já existe, não inicializa de novo
            if (this.map) {
                console.warn('⚠️ Mapa já foi inicializado - pulando reinicialização');
                return;
            }

            // ✅ LIMPA CONTAINER DO LEAFLET se já foi usado antes
            const mapElement = this.mapElement;
            if (mapElement._leaflet_id) {
                console.warn('⚠️ Container já tem Leaflet - limpando...');
                mapElement._leaflet_id = null;
                mapElement.innerHTML = ''; // Limpa conteúdo
            }
            ...
            const baseLayers = {
                ...
                '🌍 Satélite + Ruas': L.tileLayer('https://mt{s}.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', {
            ...
            // Foca na região amazônica MAS permite navegar pelo mundo todo
            const amazonBounds = L.latLngBounds(
                [-15, -75], // Sudoeste da Amazônia
                [5, -44]    // Nordeste da Amazônia
            );
            // ✅ REMOVIDO: this.map.setMaxBounds() - agora permite explorar o mundo todo
            // O mapa INICIA na Amazônia mas você pode navegar livremente!
            ...
            // ✅ CORREÇÃO CRÍTICA: Aguarda mapa estar montado e força recálculo de dimensões
            setTimeout(() => {
                if (this.map) {
                    console.log('🔄 Forçando recálculo de dimensões do mapa...');
                    this.map.invalidateSize();

                    // Agora sim carrega os dados REAIS
                    console.log('📡 Iniciando carregamento de dados da Amazônia...');
                    this.loadRealAmazonData();
                } else {
                    console.error('❌ ERRO: Mapa não foi criado corretamente');
                }
            }, 200); // Aguarda 200ms para garantir que mapa está montado

        } catch (error) {
            console.error('❌ ERRO CRÍTICO ao inicializar mapa:', error);
            console.error('   Stack trace:', error.stack);
            this.showMapError(
                `Erro ao inicializar mapa: ${error.message}<br><br>` +
...
// ✅ INICIALIZAÇÃO INTELIGENTE DO MAPA
// Aguarda a introdução cinemática terminar OU elemento estar visível

function initializeMapsManager() {
    console.log('🗺️ Tentando inicializar MapsManager...');

    // ✅ VERIFICAÇÃO CRÍTICA: Se já existe, não cria de novo
    if (window.mapsManager) {
        console.warn('⚠️ MapsManager já existe - pulando inicialização');
        return;
    }
    ...
    if (!isVisible) {
        console.warn('⚠️ Elemento #interactive-map existe mas está OCULTO');
        console.warn('   Dimensões:', rect.width, 'x', rect.height);
        console.warn('   Aguardando ficar visível...');
        return; // Não inicializa se oculto
    }

    // Se chegou aqui, elemento existe E está visível
    console.log('✅ Elemento #interactive-map pronto - criando MapsManager');

    try {
        window.mapsManager = new MapsManager();
        console.log('✅ MapsManager criado com sucesso');
    } catch (error) {
        console.error('❌ ERRO ao criar MapsManager:', error);
        console.error('   Stack:', error.stack);
    }
}

// Estratégia 1: Aguarda evento customizado da introdução cinemática
console.log('📡 Aguardando evento "cinematicIntroComplete"...');
document.addEventListener('cinematicIntroComplete', () => {
    console.log('🎬 Introdução cinemática completa - inicializando mapa');
    setTimeout(initializeMapsManager, 300); // Pequeno delay para garantir
});

// Estratégia 2: Fallback com polling se introdução demorar muito
let pollAttempts = 0;
const maxPollAttempts = 40; // 20 segundos máximo

function pollForVisibleElement() {
    pollAttempts++;

    // Se já foi inicializado, para de polling
    if (window.mapsManager) {
        console.log('✅ Mapa já inicializado - parando polling');
        return;
    }

    const mapElement = document.getElementById('interactive-map');
    if (!mapElement) {
        console.warn(`⏳ Tentativa ${pollAttempts}/${maxPollAttempts}: Elemento não encontrado`);

        if (pollAttempts < maxPollAttempts) {
            setTimeout(pollForVisibleElement, 500);
        } else {
            console.error('❌ TIMEOUT: Elemento #interactive-map nunca apareceu');
        }
        return;
    }
    ...
    if (isVisible) {
        console.log(`✅ Elemento visível após ${pollAttempts} tentativas (${pollAttempts * 0.5}s)`);
        initializeMapsManager();
    } else if (pollAttempts < maxPollAttempts) {
        setTimeout(pollForVisibleElement, 500);
    } else {
        console.error('❌ TIMEOUT: Elemento #interactive-map nunca ficou visível');
        console.error('   Última dimensão detectada:', rect.width, 'x', rect.height);
        ...
        if (mapEl) {
            mapEl.innerHTML = `
                ...
                ">
                    <div>
                        <div style="font-size: 48px; margin-bottom: 10px;">🗺️</div>
                        <h3 style="margin: 0 0 10px 0; color: #d32f2f;">Erro no Mapa</h3>
                        <p style="margin: 0; color: #666;">
                            Elemento do mapa não ficou visível após 20 segundos.<br>
                            Possível problema com a introdução cinemática.
                        </p>
                        <button onclick="window.location.reload()" style="
                            margin-top: 15px;
                            padding: 8px 16px;
...
// Inicia polling após 2 segundos (dá tempo para intro começar)
window.addEventListener('load', () => {
    console.log('📄 Página carregada - iniciando polling do mapa em 2s...');
    setTimeout(pollForVisibleElement, 2000);
});
