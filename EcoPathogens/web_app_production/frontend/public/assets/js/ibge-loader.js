...
class IBGEDataLoader {
    constructor() {
        ...
        this.isLoaded = false;
        this.baseUrl = 'data/';
        // CDN R2 para arquivos grandes
        this.cdnUrl = 'https://media.oclubecoromandel.com.br/';
    }
    ...
    async loadAmazoniaLegal(simplified = true) {
        try {
            ...
            // Versão simplificada local, completa do CDN R2
            let url;
            if (simplified) {
                url = this.baseUrl + 'amazonia_legal_simplified.geojson';
            } else {
                // Arquivo grande carregado do Cloudflare R2 CDN
                url = this.cdnUrl + 'amazonia_legal_ibge.geojson';
            }
